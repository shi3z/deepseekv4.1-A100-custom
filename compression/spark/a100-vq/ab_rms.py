...
print(f"gate={FD.GATE_KERNEL} qkv={FD.FUSE_QKV} swiglu={EF.SWIGLU} merge={EF.MERGE} "
      f"rmsnorm={EF.RMSNORM}", flush=True)
...
def checking(x, w, eps):
    a = orig.__wrapped__(x, w, eps) if hasattr(orig, "__wrapped__") else None
    EF.RMSNORM = False
    ref = orig(x, w, eps)
    EF.RMSNORM = True
    new = orig(x, w, eps)
...
print("\ninterleaved timing (A = torch rmsnorm, B = fused)")
