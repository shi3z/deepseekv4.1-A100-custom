...
vqa = F8.CBF8Arena(S, dev).attach(CBF8(dev))
...
base_up, base_dn = F8.CBF8_UP_CFG[BM], F8.CBF8_DOWN_CFG[BM]
print("cb3 v3", round(bench(lambda: C3.moe_forward_v3(x, slots, wts, cb, 10.0)), 3),
      "cbf8 base", round(bench(lambda: F8.moe_forward_cbf8(x, slots, wts, vqa, 10.0)), 3))
for which in ("up", "down"):
    ...
    for bn, nw, ns in itertools.product((16, 32, 64, 128), (1, 2, 4, 8), (1, 2, 3, 4)):
        ...
        rows.append((bench(lambda: F8.moe_forward_cbf8(x, slots, wts, vqa, 10.0, cfg_up=u, cfg_down=d)), cfg))
    rows.sort()
