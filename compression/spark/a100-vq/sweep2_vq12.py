...
base_up, base_dn = VQM._VQ_UP_CFG[BM], VQM._VQ_DOWN_CFG[BM]
print("cb3 v3", round(bench(lambda: C3.moe_forward_v3(x, slots, wts, cb, 10.0)), 3),
      "vq12 base", round(bench(lambda: VQM.moe_forward_vq(x, slots, wts, vqa, 10.0)), 3))
for which in ("up", "down"):
    ...
    for bn, nw, ns in itertools.product((16, 32, 64, 128), (1, 2, 4, 8), (1, 2, 3, 4)):
        ...
        rows.append((bench(lambda: VQM.moe_forward_vq(x, slots, wts, vqa, 10.0, cfg_up=u, cfg_down=d)), cfg))
    rows.sort()
