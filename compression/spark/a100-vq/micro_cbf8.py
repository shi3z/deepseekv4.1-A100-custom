"""CB3 v3 vs CBF8: correctness against the format's own dequant, then time."""
import os, sys, time
import torch
...
import cb3_moe as C3, fp4_moe as F4, vq12_moe as VQM, vq6_moe as V6, cbf8_moe as F8
...
vqa = VQM.VQ12Arena(S, dev).attach(VQ12(os.path.expanduser("~/dsv41-spark/a100-vq/vq_3.0.npz"), dev))
v6a = V6.VQ6Arena(S, dev).attach(VQ6(os.path.expanduser("~/dsv41-spark/a100-vq/vq2_3.npz"), dev))
for i in range(S):
    args = (w(INTER, DIM), sc(INTER, DIM), w(DIM, INTER), sc(DIM, INTER), w(INTER, DIM), sc(INTER, DIM))
    cb.load_slot(i, *args); f8.load_slot(i, *args); vqa.load_slot(i, *args); v6a.load_slot(i, *args)
...
def ref(arena):
    ...
    for t in range(T):
        for kk in range(K):
            ...
            out[t] += float(wts[t, kk]) * (hh @ w2.float().T)
    return out

r = ref(f8)
y = F8.moe_forward_cbf8(x, slots, wts, f8, 10.0).float()
print(f"CBF8 kernel vs its own dequant: rel err {float((y-r).norm()/r.norm()):.2e}")
...
print(f"T={T} K={K}:  CB3 v3 {a:.3f}   CBF8 {d:.3f} ({(d-a)/a*100:+.1f} %)   "
      f"VQ6 {c:.3f} ({(c-a)/a*100:+.1f} %)   VQ12 {b:.3f} ({(b-a)/a*100:+.1f} %)")
