...
export DSV41_CACHE_GROW_CHUNK=1024
...
mkdir -p results
/home/shi3z/.local/bin/python -u -m dsv41.serve \
    ...
    --host 0.0.0.0 \
    --port 8000 \
    --mtp 0 2>&1 | tee -a results/server.log

EXIT_CODE=$?
echo "[server] Server process exited with code $EXIT_CODE at $(date)" | tee -a results/server.log
sleep 3600
