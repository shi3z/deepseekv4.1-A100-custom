#!/usr/bin/env python3
"""Verification suite for Claude Code compatibility through LiteLLM.

Tests:
  1. Multi-turn conversation with shared prefix (tests GPU LCP cache hit)
  2. Diverse prompts without shared prefix
  3. Concurrent worker requests (tests batching and slot allocation)
  4. Anthropic tool calling format (tests schema translation and tool_use blocks)
"""
...
LITELLM_URL = os.environ.get("LITELLM_URL", "http://127.0.0.1:8101/v1/messages?beta=true")
MODEL_NAME = os.environ.get("MODEL_NAME", "deepseek-v4.1-flash")


def post_json(url: str, data: dict, headers: dict | None = None) -> tuple[int, str]:
...
def test_tool_calling():
    ...
    assert any(b.get("type") == "tool_use" for b in content_blocks), "Expected tool_use block in response"
